...
function findTenantIdByName_(tenantDisplayName) {
  ...
  const found = rows.find(function (r) {
    return (
      String(r["Tenant"]).trim().toLowerCase() ===
      String(tenantDisplayName).trim().toLowerCase()
    );
  });
...
function getFarmFilterData(systemName) {
  try {
    const requestedSystem = String(systemName || "FMS")
      .trim()
      .toUpperCase();
    ...
    rows.forEach(function (r) {
      const managedRaw = String(
        r["Managed by"] || r["Managed_by"] || "",
      ).trim();
      const managedParsed =
        _parseFarmFromFlockPattern_(managedRaw) || managedRaw;
    ...
    Object.keys(flocksByFarm).forEach(function (farm) {
      flocksByFarm[farm].sort(function (a, b) {
        return String(a["FLOCK_ID"] || "").localeCompare(
          String(b["FLOCK_ID"] || ""),
        );
      });
    });
...
function _deriveFarmNameFromConfigRow_(row) {
  const managedByRaw = String(
    row["Managed by"] || row["Managed_by"] || "",
  ).trim();
