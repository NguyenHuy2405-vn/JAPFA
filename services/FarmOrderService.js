...
function getFarmOrderInfo(flockId) {
  try {
    ...
    if (!cfg) {
      throw new Error("NO_CONFIG::Không tìm thấy cấu hình cho flock đã chọn.");
    }

    const current = _getCurrentFmsRow_(flockId);
    if (!current) {
      throw new Error("NO_FMS::Chưa có dữ liệu FMS cho flock đã chọn.");
    }
...
function calcSuggestedQty(flockId, storageDays) {
  try {
    ...
    if (isNaN(storage) || storage < 0 || Math.floor(storage) !== storage) {
      throw new Error(
        "INVALID_STORAGE_DAYS::Storage Days phai la so nguyen >= 0.",
      );
    }
    ...
    if (suggestedQty === null || suggestedQty === undefined) {
      throw new Error(
        "NO_REF_DATE_QTY::Khong tim thay FEED_end_qtty tai ngay tham chieu.",
      );
    }
    ...
    const infoMessage = isEnoughUntilRefDate
      ...
      : suggestedOrderQtyKg > 0
        ? "Can de xuat dat " +
          suggestedOrderQtyKg +
          " kg de bu thieu den ngay tham chieu."
...
function calcSuggestedBagQty(suggestedQtyKg, packagingSku) {
  try {
    ...
    if (isNaN(qty) || qty <= 0) {
      throw new Error(
        "INVALID_SUGGESTED_QTY::Suggested Quantity phai lon hon 0.",
      );
    }
    ...
    if (!product) {
      throw new Error(
        "NO_PACKAGING_SKU::Khong tim thay Packaging SKU da chon.",
      );
    }
    ...
    if (isNaN(uomWeight) || uomWeight <= 0) {
      throw new Error(
        "INVALID_UOM_WEIGHT::Thieu hoac sai UOM_weight_kg cua Packaging SKU.",
      );
    }
