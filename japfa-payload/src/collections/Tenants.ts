...
import { adminOnly, denyAll, tenantRecordScoped } from "@/access/permissions";
import { writeAudit } from "@/services/audit/audit.service";

export const Tenants: CollectionConfig = {
  slug: "tenants",
  access: {
    read: tenantRecordScoped,
    create: adminOnly,
    update: adminOnly,
    delete: denyAll,
  },
  ...
  fields: [
    ...
    },
    {
      ...
      required: true,
      unique: true,
      label: "Tenant ID (Ví dụ: WMS_1, WMS_01)",
    },
    {
      name: "name",
      type: "text",
      required: true,
      label: "Tên hiển thị (Ví dụ: Japfa Farm 1, Nhà máy Bình Dương)",
    },
    {
      name: "system",
      type: "select",
      ...
      async ({ doc, previousDoc, req, operation }) => {
        ...
        try {
          if (doc.status === "LOCKED" && previousDoc?.status !== "LOCKED") {
            await writeAudit(req.payload, { actorUserId: actor.id, actorEmail: actor.email, actorRole: actor.role, action: "FARM_LOCKED", targetCollection: "tenants", targetId: doc.id, before: { status: previousDoc?.status }, after: { status: doc.status } });
          }
          if (doc.status === "ACTIVE" && previousDoc?.status === "LOCKED") {
            await writeAudit(req.payload, { actorUserId: actor.id, actorEmail: actor.email, actorRole: actor.role, action: "FARM_UNLOCKED", targetCollection: "tenants", targetId: doc.id, before: { status: previousDoc?.status }, after: { status: doc.status } });
          }
        } catch (error) {
